...
namespace PackageSellSystemTrading{
    public class Xing_t1857Buy : XAQueryClass{
        ...
        public Xing_t1857Buy(){
            ...
            searchListDt.Columns.Add("시장구분"     , typeof(string));
		...
		void receiveDataEventHandler(string szTrCode){
            ...
            try {
                ...
                for (int i = 0; i < iCount; i++) {
                    ...
                    {
                        foundRows = this.searchListDt.Select("종목코드 = '" + shcode + "'");
                    }
                    //종목이 없으면 추가 있으면 수정
                    ...
                    //시장구분
                    t8436Rows = mainForm.xing_t8436.getT8436DataTable().Select("shcode = '" + shcode + "'");
                    if (t8436Rows.Count() > 0)
                    {
                        t8436Row = t8436Rows[0];
                        tmpRow["시장구분"] = t8436Row["gubun"];//시장구분(1:코스피 2:코드닥)
                    }
        ...
        private Boolean execute()
        {
            ...
            Boolean kosBuyAt = Util.isKosBuyAt(mainForm.real_IJ.ks_drate, mainForm.real_IJ.ks_change);
            Boolean kodBuyAt = Util.isKodBuyAt(mainForm.real_IJ.kd_drate, mainForm.real_IJ.kd_change); 
          
            //3.검색목록 매수 테스트
            int rowIndex = 0;
            foreach (DataRow itemRow in this.searchListDt.AsEnumerable())
            {
                executeTest(itemRow, configSearchCode, nowTimeSpan, rowIndex, kosBuyAt, kodBuyAt);
                rowIndex++;
            }
            return true;
        }
